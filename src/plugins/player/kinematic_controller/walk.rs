...
///   that  will not attempt to fix the character's rotation along the up axis.
///
...
///   prevents that.
#[derive(Clone)]
pub struct BuiltinWalk {
    /// The direction (in the world space) and speed to accelerate to.
    ///
    ///  assumes that this vector is orthogonal to the up dierction.
    pub desired_velocity: Vec3,

    /// If non-zero,  will rotate the character so that its negative Z will face in that
    /// direction.
    ///
    ///  assumes that this vector is orthogonal to the up direction.
    ...
    /// [`BuiltinCrouch`](crate::builtins::BuiltinCrouch) action.
    ...
    /// above the `float_height`,  will consider it to be in the air.
...
impl Basis for BuiltinWalk {
    const NAME: &'static str = "BuiltinWalk";
    type State = BuiltinWalkState;
    ...
    fn apply(&self, state: &mut Self::State, ctx: BasisContext, motor: &mut Motor) {
        ...
        let slipping_vector: Option<Vec3>;
        ...
        let upward_impulse: VelChange = 'upward_impulse: {
                ...
                match &mut state.airborne_timer {
                    ...
                    Some(_) => {
                        ...
                        if state.vertical_velocity <= 0.0 {
                            break 'upward_impulse VelChange::acceleration(
                                -self.free_fall_extra_gravity * ctx.up_direction,
                            );
                        } else {
                            break 'upward_impulse VelChange::ZERO;
                        }
                    }
                }
            }
            error!(" could not decide on jump state");
            VelChange::ZERO
        };

        motor.lin = walk_vel_change + VelChange::boost(impulse_to_offset) + upward_impulse;
        let new_velocity = state.effective_velocity
            + motor.lin.boost
            + ctx.frame_duration * motor.lin.acceleration
            - impulse_to_offset;
        state.running_velocity = new_velocity.reject_from(ctx.up_direction);
        ...
        let torque_to_fix_tilt = {
            ...
            let rotation_required_to_fix_tilt =
                Quat::from_rotation_arc(tilted_up, ctx.up_direction);
        ...
        let existing_angvel = ctx.tracker.angvel.dot(ctx.up_direction);
...
impl BuiltinWalk {
    ...
    /// [`BuiltinCrouch`](crate::builtins::BuiltinCrouch) may rely on it.
    pub fn spring_force(
        &self,
        state: &BuiltinWalkState,
        ctx: &BasisContext,
        spring_offset: f32,
    ) -> VelChange {
