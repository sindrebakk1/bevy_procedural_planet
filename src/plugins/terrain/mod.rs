    ...
    [(); (SUBDIVISIONS + 1).pow(2) * 6]:,
{
    ...
    // 
    // for (cube_tree, grid, grid_cell, transform, radius, mut cache) in planet_query.iter_mut() {
    //     for chunk in cube_tree.iter() {
    ...
    //                 grid.translation_to_grid(node.center_unchecked() - planet_pos);
    //             let chunk_entity = commands
    //                 .spawn_empty()
    //                 .set_parent(entity)
    //                 .insert((Chunk, grid_cell, Transform::from_translation(translation)))
    //                 .id();
    // 
